...
def path_decompose2(a,b,a_true,b_true,overwrite_norm,P,use_GLPK, sparsity= False):
    ...
    **Do not use over_write_norm becuase a_true and b_true surrently have normalization instead of copycount. 
    ...
    ########## 2nd trial
    #change: if len(mask_indice)>0: to if 1: use formulation 2
    #change: if len(mask_indice)>0: to if 0: use original path decompose (recommended)
    #keep:   if len(mask_indice)>0: use combination of formulation 2 and original path decompose
    #
    #if formulation 2 is used, c[i] = c[i] is worse than c[i] = c[i]*p[i] (Pij=1,cij=0)
        ...
        for i in range(m):
            for j in range(n):
                ...
                if another_sol[j*m+i]<0:
                    another_sol[j*m+i]=0
                    temp_sol[j*m+i]=0
        #print('temp_sol(after thresholding)=\n%s'%str(temp_sol))
        #pdb.set_trace()
...
if __name__ == '__main__':
    ...
    [a2,z] = path_decompose2(a,b,a,b,False,P,False,10)
