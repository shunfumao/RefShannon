...
numpy, math, heapq, random, re| common python packages
pyfaidx | used to load genome
...
-g genome_file | optional. to be used when final transcript output includes both gtf and fasta files. specifies the genome file to use. genome_file should be in fasta format and is absolute path. It can be single genome or multi genome.
...
--outputFasta | optional. to be used when final transcript output includes both gtf and fasta files. To be consistent with -g option.
...
By running
`python refShannon.py --batch -i path2/hits.sam -g path1/chr15.fa -paired -target chr15 -N 20 --outputFasta`,
...
-g multi_genome_file | optional. to be used when final transcript output includes both gtf and fasta files. specifies the multi genome file to use. multi_genome_file should be in fasta format and is absolute path. This file should contain all the chromosomes that the alignment files are generated from.
...
--outputFasta | (same as in command 1)
...
Then by running
`python refShannon.py --batch -I path3/ -g path1/hg19.fa -O path4/ -chrs chr1,chr15 -paired -N 20 --outputFasta`
...
Then by running
`python refShannon.py --batch -I path3/ -G path1/ -O path4/ -chrs chr1,chr15 -paired -N 20 --outputFasta`
...
2017.12.28 - make output of fasta as optional
