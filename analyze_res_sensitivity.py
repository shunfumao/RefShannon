    ...
    if exp_format=='KAL':
        ...
        for lines in open(exp_file):
            ...
            try:
                ...
                tr_ab = float(fields[5])/1e6;
                tr_cov = float(fields[4]) / max(1,(tr_len-L)) * L # * 20 / 135
                tot_ab = tot_ab + tr_ab
                tr_dict_full[tr_id] = [tr_len, tr_cov, tr_ab, 0, 0, 0] #last 4 entries are num_iso, ours, stringtie
            except ValueError:
                pdb.set_trace()
...
(1) python analyze_res_sensitivity.py  --findCoveredTr \
                                   --et end_threshold \
...
(2) python analyze_res_sensitivity.py  --findTrGeneNum \
                                       -i path/to/rsem_exp_isoforms_results \
...
(3) python analyze_res_sensitivity.py --performancePlotExpress \
